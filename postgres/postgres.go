...
const postHdrLen = 9
const postMaxLen = 268435456

const unknownPreparedQueryPath = "/unknown-prepared-query"

func (rule *PostgresRule) Matches(data interface{}) bool {
	...
	if rule.queryActionExact != "" && rule.queryActionExact != parts[2] {
		log.Debugf("PostgresRule: query_action mismatch %v, %s", rule.queryActionExact, parts[1])
		return false
	}
	if len(parts[3]) > 0 &&
		rule.tableRegexCompiled != nil &&
		!rule.tableRegexCompiled.MatchString(parts[3]) {
		log.Debugf("PostgresRule: table_regex mismatch '%v', '%s'", rule.tableRegexCompiled, parts[3])
		return false
	}
...
// postgresRuleParser parses protobuf L7 rules to enforcement objects
// May panic
func PostgresRuleParser(rule *cilium.PortNetworkPolicyRule) []L7NetworkPolicyRule {
	...
	for _, l7Rule := range allowRules {
		...
		if len(cr.queryActionExact) > 0 {
			...
			if res == invalidAction {
				ParseError(fmt.Sprintf("Unable to parse L7 postgres rule with invalid query_action: '%s'", cr.queryActionExact), rule)
...
var postgresParserFactory *PostgresParserFactory

func init() {
	log.Debug("init(): Registering postgresParserFactory")
	RegisterParserFactory("postgres", postgresParserFactory)
	RegisterL7RuleParser("postgres", PostgresRuleParser)
}
...
func (p *PostgresParser) OnData(reply, endStream bool, dataArray [][]byte) (OpType, int) {
	...
	if reply {
		...
		postgresParseReply(p, data[0:(postHdrLen+requestLen)])

		log.Debugf("reply, passing %d bytes", (postHdrLen + requestLen))
		return PASS, (postHdrLen + int(requestLen))
	}

	err, paths := postgresParseRequest(p, data[0:(postHdrLen+requestLen)])
	if err != 0 {
	...
	if !matches {
		...
		return DROP, int(postHdrLen + requestLen)
	}

	return PASS, int(postHdrLen + requestLen)
}
...
func parseQuery(p *PostgresParser, query string) (string, string) {
	var action string
	var table string
...
func postgresParseRequest(p *PostgresParser, data []byte) (OpError, []string) {
...
func postgresParseReply(p *PostgresParser, data []byte) {
