...
func (s *PostgresSuite) SetUpSuite(c *C) {
...
func (s *PostgresSuite) checkAccessLogs(c *C, expPasses, expDrops int) {
...
func (s *PostgresSuite) TearDownTest(c *C) {
	s.logServer.Clear()
}

func (s *PostgresSuite) TearDownSuite(c *C) {
	s.logServer.Close()
}
...
func (s *PostgresSuite) TestPostgresOnDataNoHeader(c *C) {
...
func (s *PostgresSuite) TestPostgresOnDataOptionsReq(c *C) {
	s.ins.CheckInsertPolicyText(c, "1", []string{`
		...
		>
		`})
	conn := s.ins.CheckNewConnectionOK(c, "postgres", true, 1, 2, "1.1.1.1:34567", "2.2.2.2:80", "cp6")
...
func (s *PostgresSuite) TestPostgresOnDataPartialReq(c *C) {
	s.ins.CheckInsertPolicyText(c, "1", []string{`
		name: "cp5"
		policy: 2
		ingress_per_port_policies: <
		...
		>
		`})
	conn := s.ins.CheckNewConnectionOK(c, "postgres", true, 1, 2, "1.1.1.1:34567", "2.2.2.2:80", "cp5")
...
func (s *PostgresSuite) TestPostgresOnDataQueryReq(c *C) {
	s.ins.CheckInsertPolicyText(c, "1", []string{`
		name: "cp4"
		policy: 2
		ingress_per_port_policies: <
		...
		>
		`})
	conn := s.ins.CheckNewConnectionOK(c, "postgres", true, 1, 2, "1.1.1.1:34567", "2.2.2.2:80", "cp4")
...
func (s *PostgresSuite) TestPostgresOnDataSplitQueryReq(c *C) {
	s.ins.CheckInsertPolicyText(c, "1", []string{`
		name: "cp3"
		policy: 2
		ingress_per_port_policies: <
		...
		>
		`})
	conn := s.ins.CheckNewConnectionOK(c, "postgres", true, 1, 2, "1.1.1.1:34567", "2.2.2.2:80", "cp3")
...
func (s *PostgresSuite) TestPostgresOnDataMultiReq(c *C) {
	s.ins.CheckInsertPolicyText(c, "1", []string{`
		name: "cp2"
		policy: 2
		ingress_per_port_policies: <
	...
	conn := s.ins.CheckNewConnectionOK(c, "postgres", true, 1, 2, "1.1.1.1:34567", "2.2.2.2:80", "cp2")
...
func (s *PostgresSuite) TestSimplePostgresPolicy(c *C) {
	s.ins.CheckInsertPolicyText(c, "1", []string{`
		name: "cp1"
		policy: 2
		ingress_per_port_policies: <
	...
	conn := s.ins.CheckNewConnectionOK(c, "postgres", true, 1, 2, "1.1.1.1:34567", "2.2.2.2:80", "cp1")
...
func (s *PostgresSuite) TestPostgresBatchRequestPolicy(c *C) {
	s.ins.CheckInsertPolicyText(c, "1", []string{`
		name: "cp1"
		policy: 2
		ingress_per_port_policies: <
...
func (s *PostgresSuite) TestPostgresBatchRequestPolicyDenied(c *C) {
	s.ins.CheckInsertPolicyText(c, "1", []string{`
		name: "cp1"
		policy: 2
		ingress_per_port_policies: <
...
func (s *PostgresSuite) TestPostgresBatchRequestPreparedStatement(c *C) {
	s.ins.CheckInsertPolicyText(c, "1", []string{`
		name: "cp1"
		policy: 2
		ingress_per_port_policies: <
	...
	cassParser, ok := (conn.Parser).(*PostgresParser)
	if !ok {
		panic("failed to cast conn.Parser to *PostgresParser\n")
	}
...
func (s *PostgresSuite) TestPostgresBatchRequestPreparedStatementDenied(c *C) {
	s.ins.CheckInsertPolicyText(c, "1", []string{`
		name: "cp1"
		policy: 2
		ingress_per_port_policies: <
	...
	postParser, ok := (conn.Parser).(*PostgresParser)
	if !ok {
		panic("failed to cast conn.Parser to *PostgresParser\n")
	}
	preparedQueryID1 := "aaaa"
	postParser.preparedQueryPathByPreparedID[preparedQueryID1] = "/batch/select/db3.t1"
	preparedQueryID2 := "bbbb"
	postParser.preparedQueryPathByPreparedID[preparedQueryID2] = "/batch/select/db4.t2"
...
func (s *PostgresSuite) TestPostgresExecutePreparedStatement(c *C) {
	s.ins.CheckInsertPolicyText(c, "1", []string{`
		name: "cp1"
		policy: 2
		ingress_per_port_policies: <
	...
	preparedQueryID1 := "aaaa"
	postParser.preparedQueryPathByPreparedID[preparedQueryID1] = "/query/select/db3.t1"
...
func (s *PostgresSuite) TestPostgresExecutePreparedStatementUnknownID(c *C) {
...
func (s *PostgresSuite) TestPostgresPreparedResultReply(c *C) {
	...
	postParser.preparedQueryPathByStreamID[uint16(4)] = "/query/select/db3.t1"
...
func (s *PostgresSuite) TestPostgresAdditionalQueries(c *C) {
	s.ins.CheckInsertPolicyText(c, "1", []string{`
		name: "cp1"
		policy: 2
		ingress_per_port_policies: <
...
func (s *PostgresSuite) TestPostgresUseQuery(c *C) {
	s.ins.CheckInsertPolicyText(c, "1", []string{`
		name: "cp1"
		policy: 2
		ingress_per_port_policies: <
